...
function Request_admin() {
  ...
  const filteredHistory = History.filter((data) => data["Type of Request"] === selectedTab);
 ...
 useEffect(()=>{setTotalPages(filteredAndSearchedHistory.length/itemsPerPage)},[selectedTab,search])
 const [buttonTextArray, setButtonTextArray] = useState(Array(History.length).fill("Approve"));

  const RenderHistory = ({ onCheckUpdates , data}) => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    const handleButtonClick = (index) => {
      const newButtonTextArray = [...buttonTextArray];
      newButtonTextArray[index] = buttonTextArray[index] === "Approve" ? "Withdraw" : "Approve";
      setButtonTextArray(newButtonTextArray);
    };
    return data.slice(startIndex, endIndex).map((data, index) => {
      ...
      return (
        ...
        >
          <div className="text-xs text-[#494D57] w-[7%] text-center py-5">
            {sequenceNumber}
          </div>
          ...
          <div className={`text-xs text-center w-[18%] py-5`}>
          <button
            className={`text-sm  h-8 w-20 ${
              buttonTextArray[index] === "Approve"
                ? "bg-[#2164E8]  text-white"
                : "text-black border border-[#767A81]"
            }  rounded`}
...
<div className="flex items-center gap-2 lg:gap-4 flex-col lg:flex lg:flex-row ">
  <div className=" text-sm font-semibold text-[#1E2532] ">Requests Approved by you : 12 </div>
  <div className=" text-xs font-semibold text-[#666b78]">Total Requests received : {History.length} </div>
            ...
            <div className=" flex flex-col gap-[3px] ">
              <div className="flex mt-4 bg-[#E8E9EA] items-center  shadow-[0px_1.6px_3.6px_0px_rgba(27,33,45,0.13),0px_0.3px_0.9px_0px_rgba(27,33,45,0.10)]">
                ...
                <div className=" text-sm lg:w-[20%]  justify-center  gap-[3px] flex items-center py-3">
                  Request Name
                  <img src="/sort.svg" />
                </div>
                <div className=" text-sm  lg:w-[15%] text-center   py-3">
                  Request Sender
                </div>
                ...
                <div className=" text-sm  lg:w-[18%]  text-center  py-3">
                  Approve/Withdraw
                </div>
              </div>
             {authorizationObject[selectedTab]?<> <RenderHistory onCheckUpdates={handleCheckUpdates} data={filteredAndSearchedHistory} />
      ...
      <div className="flex justify-center py-10 text-xl font-extrabold text-[#7a7e87]"> You have not Authorized for approving {selectedTab}</div>
    }
            </div>
          </>
        )}
