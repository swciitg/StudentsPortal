...
<label className="flex flex-col gap-2 ">
  <span className="font-medium text-sm">Password</span>
    <input onChange={(e)=>setpass(e.target.value)}  className="border p-2 pt-[5px] pb-[5px] text-black outline-none rounded-md border-[rgba(118,122,129,1)] pl-3" type="password" placeholder="Enter Password"/>
</label>
<label className="flex flex-col gap-2 ">
  <span className="font-medium text-sm">Confirm Password</span>
    <input onChange={(e)=>setconfpaas(e.target.value)}  className="border p-2 pt-[5px] pb-[5px] text-black outline-none rounded-md border-[rgba(118,122,129,1)] pl-3" type="password" placeholder="Rewrite Password "/>
</label>
</div>
       { paas===confpass&&paas.length>0?
        <div className="flex justify-end mt-10"><Link to='/'><button className=" inline-flex items-center p-1 bg-[#2164E8] text-white rounded-sm pl-4 pr-4">Submit</button></Link></div> 
